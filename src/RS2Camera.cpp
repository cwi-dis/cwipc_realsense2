...
void RS2Camera::_processing_thread_main()
{
	...
	while(!stopped) {
		...
		if (do_depth_filtering) {
			...
			if (camera_processing.do_decimation) processing_frameset = processing_frameset.apply_filter(dec_filter);
			if (camera_processing.do_threshold) processing_frameset = processing_frameset.apply_filter(threshold_filter);
			if (camera_processing.do_spatial || camera_processing.do_temporal) {
				...
				if (camera_processing.do_spatial) processing_frameset = processing_frameset.apply_filter(spat_filter);
				if (camera_processing.do_temporal) processing_frameset = processing_frameset.apply_filter(temp_filter);
