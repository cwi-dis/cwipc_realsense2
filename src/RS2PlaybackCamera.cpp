...
void RS2PlaybackCamera::pause() {
    rs2::device dev = _camera_pipeline.get_active_profile().get_device();
...
void RS2PlaybackCamera::_post_start_this_camera(rs2::pipeline_profile& profile) {
    ...
    if (_camera_config.playback_inpoint_micros != 0) {
        uint64_t new_pos = ((uint64_t)1000) * _camera_config.playback_inpoint_micros;
...
void RS2PlaybackCamera::post_start_all_cameras() {
    rs2::pipeline_profile prof = _camera_pipeline.get_active_profile();
    rs2::device dev = prof.get_device();
    rs2::playback playback = dev.as<rs2::playback>();
    playback.resume();
    if (_debug) _log_debug("_post_start_all_cameras: playback resumed at " + std::to_string(playback.get_position()));
}
...
bool RS2PlaybackCamera::seek(uint64_t timestamp) {
    ...
    uint64_t pos = _camera_config.playback_inpoint_micros * 1000;
