...
// Static variable used to print a warning message when we re-create an RS2BaseCapture
...
void RS2BaseCapture::_setup_camera_hardware_parameters() {
    rs2::device_list devs = capturer_context.query_devices();
    for (auto dev : devs) {
...
void RS2BaseCapture::_setup_camera_sync() {
...
void RS2BaseCapture::_find_camera_positions() {
...
bool RS2BaseCapture::_create_cameras() {
    rs2::device_list devs = capturer_context.query_devices();
    const std::string platform_camera_name = "Platform Camera";
...
void RS2BaseCapture::_control_thread_main() {
    ...
    while(!stopped) {
        ...
        if (configuration.debug) std::cerr << "RS2BaseCapture: creating pc with ts=" << timestamp << std::endl;
...
void RS2BaseCapture::merge_camera_pointclouds() {
...
RS2CameraConfig* RS2BaseCapture::get_camera_config(std::string serial) {
    for (int i = 0; i < configuration.all_camera_configs.size(); i++) {
